...
function ValueCard({ value, index, isViewportActive, cardRef }: { value: (typeof values)[0]; index: number; isViewportActive: boolean; cardRef: (el: HTMLDivElement | null) => void }) {
  ...
  return (
    <div
      ref={cardRef}
      className={`glass-panel border-white/5 bg-white/5 rounded-3xl p-8 sm:p-10 relative overflow-hidden group transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl ${isViewportActive ? "viewport-active" : ""}`}
      style={{
